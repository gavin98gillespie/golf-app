...
> **Latest direction — September 24:** Navigation is now four evenly spaced buttons: Home, Play, Games, Me. Play opens the round sheet; Games uses a circular trophy icon. Games is a picker with individually illustrated game buttons and a matching rivalry ledger button; round actions appear only inside a selected game. Game detail displays an inline cream rules scorecard and fixed existing/new-round actions. Existing rounds use an on-demand picker. See [Games and rules implementation](2026-09-24-games-and-rules.md). Home contains inline golfer/course search; Me retains its ledger shortcut. Signup is one basic form with transactional profile creation, and mandatory onboarding is retired. See [simplified navigation and validation](2026-09-24-simple-navigation.md).
