...
> **Latest direction — September 24:** Navigation is now four evenly spaced buttons: Home, Play, Games, Me. Play opens the round sheet; Games uses a circular trophy icon. Games shows game names and a ledger link. Game detail shows only actions and a How to play button; existing rounds are accessed through a button and an on-demand picker. See [Games and rules implementation](2026-09-24-games-and-rules.md). Home contains inline golfer/course search; Me retains its ledger shortcut. Signup is one basic form with transactional profile creation, and mandatory onboarding is retired. See [simplified navigation and validation](2026-09-24-simple-navigation.md).
